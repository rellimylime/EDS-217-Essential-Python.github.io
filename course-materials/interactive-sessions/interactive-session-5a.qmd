...
1.  Look at the `starwars` dataset (in `{dplyr}`, which is a `tidyverse` package)
...
2.  Update the YAML options so the warnings and messages are hidden, but the `library(tidyverse)` code *does* still show up in your knitted document. Knit to check.
...
<!-- Add the following chunk options: -->
...
<!-- Will only need to add `#| eval: false` since warnings are suppressed in YAML -->
...
Reference stored objects in text by adding inline R code with single backticks, a lowercase `r` between them, and then whatever you want to have show up. For example, the value below is embedded using the syntax just described:
